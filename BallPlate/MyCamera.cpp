...
int MyCamera::getBallPosition(cv::Point2d &ball_pos, cv::Mat img_raw, double XAng, double YAng) {
    ...
    cv::Matx33d rotPlate;
    rotPlate << cos(YAng),	sin(XAng)*sin(YAng),    sin(XAng) * cos(YAng),
                0,	        cos(YAng),             -sin(YAng),
               -sin(YAng),  cos(XAng)*sin(YAng),    cos(XAng) * cos(YAng);

    cv::Mat RotExInv;
    RotExInv = (Rot * rotPlate).inv();

    cv::Matx31d m_;
    m_ << circle_ball[0].val[0], circle_ball[0].val[1], 1;
...
int MyCamera::drawPoint(cv::Mat img_raw, cv::Mat &img_disp, cv::Point2d p_world, cv::Scalar color, int radius, int thickness) {
    ...
    cv::undistort(img_raw, img_disp, cameraMatrix, distCoeffs);

    cv::Matx31d M;
    M << p_world.x, p_world.y, 15;

    cv::Mat sm_;
    sm_ = cameraMatrix * (Rot * M + tvec);

    double s = sm_.at<double>(2, 0);
    
    cv::Point center;
    center.x = int(sm_.at<double>(0, 0) / s);
    center.y = int(sm_.at<double>(0, 0) / s);
